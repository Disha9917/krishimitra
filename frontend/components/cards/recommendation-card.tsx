...
export function RecommendationCard({ item, className }: RecommendationCardProps) {
  return (
    <div className={`rounded-2xl border border-slate-100 bg-white p-5 shadow-sm transition-all hover:shadow-md ${className || ""}`}>
      ...
      <div className="mt-3 space-y-3">
        ...
        <div>
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Analysis & Insights</p>
          <p className="text-sm text-slate-600 mt-0.5 leading-relaxed">{item.explanation}</p>
        </div>
        ...
        <div className="rounded-xl bg-emerald-50/60 p-3.5 border border-emerald-100/80">
          ...
          <p className="text-xs font-semibold text-emerald-950">{item.recommendedAction}</p>
          {item.expectedYieldImprovement && (
            <div className="mt-2 inline-flex items-center gap-1 rounded-md bg-emerald-600 px-2 py-0.5 text-[11px] font-bold text-white">
